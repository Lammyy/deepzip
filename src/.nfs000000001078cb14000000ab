...
do
    ...
    dedata="$data_dir/$basename.npy"
    echo dedata
    echo $dedata
    log_file="$logs_dir/$basename/$1.log.csv"
    output_dir="$compressed_dir/$basename"
    recon_file_name="$output_dir/$1.reconstructed.$extension"
    params_file="$data_dir/$basename.param.json"
    echo $params_file
    output_prefix="$output_dir/$1.compressed"
    mkdir -p "$output_dir"
    cmp $recon_file_name "$original_dir/$basename.$extension"
    status=$?
    
    echo $status
    
    if cmp $recon_file_name "$original_dir/$basename.$extension"; then
        continue 
    else
        echo "continuing"
    fi
    ...
    python trainer.py -model_name $1 -d $dedata -name $model_file -log_file $log_file 
    
    
    # Perform Compression
    echo "Starting Compression ..." | tee -a $log_file
    /usr/bin/time -v python compressor.py -data $dedata -data_params $params_file -model 
$model_file -model_name $1 -output $output_prefix -batch_size 1000 2>&1 | tee -a $log_file 
    ...
    cmp $recon_file_name "$original_dir/$basename.$extension" >> $log_file
    #echo "- - - - - "
done
